...
**Design-ruling conformance is part of every verification.** Code that works can still be wrong because it contradicts a decision the owner already made. Check the change against the recorded rulings that govern the touched area: the doc comments and commit messages of the code being changed (these often state contracts like "never pre-deploy", "nothing else needs to move", "X is the only declaration"), the PR bodies and Linear tickets the change references, and the project handoff/memory notes when provided. A change that silently contradicts a recorded ruling — or forecloses a documented intended use — is REFUTED even when every test passes; cite the ruling's source verbatim. If the diff *changes* a stated contract, verify the new statement is true rather than aspirational.
